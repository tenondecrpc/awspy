...
test.describe("editions", () => {
  test("/editions renders the empty state when only the current edition exists", async ({ page }) => {
  ...
  test("/editions/2026 mirrors the home page (current edition deep-link)", async ({ page }) => {
  ...
  test("/editions/2026/cfp shows the archived notice", async ({ page }) => {
    ...
    await expect(
      page.getByText(/esta edición ya finalizó/i)
    ).toBeVisible();
  });

  test("/editions/2026/register shows the archived notice", async ({ page }) => {
